...
>2. [힙(Heap)](https://school.programmers.co.kr/learn/courses/30/parts/12117)
> 3. 디스크 컨트롤러
...
class Solution {
    fun solution(jobs: Array<IntArray>): Int {
        ...
        while(job.isNotEmpty()){
            var j = job.filter{ it[0] <= time }
            if(j.isEmpty()){
                time += 1
                continue
            }
            var q = PriorityQueue<IntArray>{ a,b -> a[1] - b[1] }
            q.addAll(j)
            var task = q.poll()
            job = job.filter{ it[0] > time } + q.toList()
            time += task[1]
            average += time - task[0]
        }
        return average/jobs.size
    }
}
```
