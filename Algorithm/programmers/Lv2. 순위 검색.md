...
class Solution {
    fun solution(info: Array<String>, query: Array<String>): IntArray {
        ...
        fun dfs(key: String, d: Int, info: List<String>){
            if(d == 4){
                if(map.containsKey(key)) map[key]!!.add(info[d].toInt())
                else map[key] = mutableListOf(info[d].toInt())
                return
            }
            dfs("$key-", d+1, info)
            dfs("$key${info[d]}", d+1, info)
        }
        ...
        return query.map{
            var q = it.replace(" and ", "").split(" ")
            search(q[0], q[1].toInt())
        }.toIntArray()
    }
}
```
