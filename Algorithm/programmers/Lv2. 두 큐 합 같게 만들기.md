...
class Solution {
    fun solution(queue1: IntArray, queue2: IntArray): Int {
        var q1 = queue1.toMutableList()
        var q2 = queue2.toMutableList()
        
        var target = q1.sum().toLong() + q2.sum().toLong()
        if( target % 2L == 1L ) return -1
        else target /= 2L
        
        var q1Sum = q1.sum().toLong()
        var cnt = 0
        
        while( q1.isNotEmpty() && q2.isNotEmpty() ){
            if( q1Sum == target ) return cnt
